<?php

namespace PHPRapidGen\Parser;

class SlimPHPParser extends AbstractParser
{
	/**
	 * @var array shorthands for PHP Parser types
	 */
	private $types = [
		's' =>  'Scalar',
		'st' => 'Stmt',
		'h' =>  'Helper',
		't' =>  'Template',
		'n' =>  ''
	];

	/**
	 * @var \PHPParser_BuilderFactory
	 */
	private $factory;

	/**
	 * @var \PHPParser_PrettyPrinter_Default
	 */
	private $printer;

	public function __construct( $options=[] )
	{
		parent::__construct($options);

		$this->factory = new \PHPParser_BuilderFactory;

		$this->printer = new \PHPParser_PrettyPrinter_Default();
	}

	/**
	 * Convert a JSON file path into PHP code
	 *
	 * @param $source string path to the json file
	 *
	 * @return string|void
	 */
	public function parse( $source )
	{
		return $this->parseTree(
			json_decode( file_get_contents($source) )
		);
	}

	/**
	 *
	 * @param mixed $tree
	 * @param array $nodes
	 *
	 * @return string
	 */
	public function parseTree( $tree, $nodes=[] )
	{
		foreach ( $tree as $node ) {
			$nodes[] = $this->node($node);
		}

		return $this->printer->prettyPrint($nodes);
	}

	/**
	 * Multi Node Resolution
	 *
	 * @param $item
	 *
	 * @return object|string
	 */
	protected function node( $item )
	{
		if ( !is_object($item) ) {
			return parent::node($item);
		}

		$key = array_keys(get_object_vars($item))[0];

		if ( strpos($key, '.') === false ) {
			if ( $key === 'c' ) {
				return $this->resolveContext($item->$key);
			} else {
				return $this->regularNode( $item, $key, 'Expr', $key );
			}
		}

		list($t, $c) = explode('.', $key, 2);

		switch ( $t ) {
			case 't':
				return $this->template($c, $item->$key);

				break;
			case 'h':
				return $this->helper->{$c}($item->$key);

				break;
			case 'f':
				return $this->factoryNode($c, $item->$key);

				break;
			default:
				return $this->regularNode( $item, $key, self::type($t), $c );

				break;
		}
	}

	/**
	 * Factory Node resolution
	 *
	 * @param $type
	 * @param $item
	 *
	 * @return mixed
	 */
	private function factoryNode( $type, $item )
	{
		$f = $this->getFactory($type, $this->node($item[0]));

		if ( !empty($item[1]) ) {
			$sub = get_object_vars($item[1]);

			foreach ( $sub as $k => $v ) {
				if ( is_bool($v) || ($k == $v) ) {
					$f->$k();
				} elseif ( $k == 'stmts' ) {
					$f->addStmts( $this->node($v) );
				} elseif ( $k == 'params' ) {
					$f->addParams( $this->node($v) );
				} elseif ( $k == 'make' ) {
					$f->{'make'.$this->parse($v)}();
				} else {
					$f->$k($this->node($v));
				}
			}
		}

		$node = $f->getNode();

		if ( empty($item[2]) ) {
			return $node;
		}

		$attr = get_object_vars($item[2]);

		foreach ( $attr as $k => $v ) {
			if ( is_array($v) ) {
				foreach ( $v as $vp ) {
					$node->setAttribute($k, [$this->node($vp)]);
				}
			} else {
				$node->setAttribute($k, [$this->node($v)]);
			}
		}

		return $node;
	}

	/**
	 * Basic PHP Parser node type
	 *
	 * @param object $item
	 * @param string $key
	 * @param string $type
	 * @param string $class
	 *
	 * @return object
	 */
	private function regularNode( $item, $key, $type, $class )
	{
		$class = $this->getClass($class, $type);

		if ( is_array($item->$key) ) {
			$args = [];
			foreach ( $item->$key as $a ) {
				$args[] = $this->node($a);
			}
		} else {
			$args = [$this->node($item->$key)];
		}

		if ( !empty($args) ) {
			$node = $class->newInstanceArgs($args);
		} else {
			$node = $class->newInstance();
		}

		if ( !empty($item->comments) ) {
			$node->setAttribute('comments', [$this->node($item->comments[0])]);
		}

		return $node;
	}

	/**
	 * Converts the SlimPHPParser type shorthand into its proper PHPParser form
	 *
	 * @param string $type
	 *
	 * @return mixed The long form of type
	 */
	private function type( $type )
	{
		if ( isset($this->types[$type]) ) {
			return $this->types[$type];
		}

		return $type;
	}

	/**
	 * Build a factory call from a call type
	 *
	 * @param string $type
	 * @param string $name
	 *
	 * @return object generated by PHPParser factory
	 */
	private function getFactory( $type, $name )
	{
		return $this->factory->$type($name);
	}

	/**
	 * Build a full PHP Parser class name
	 *
	 * @param string $class
	 * @param string $type
	 *
	 * @return \ReflectionClass
	 */
	private function getClass( $class, $type )
	{
		$name = 'PHPParser_';

		if ( strpos($class, 'Comment') !== false ) {
			$name .= $class;
		} else {
			$name .= 'Node_';

			if ( !empty($type) ) {
				$name .= $type.'_';
			}

			$name .= $class;
		}

		return new \ReflectionClass($name);
	}
}
